...
3. No PR (`gh pr view` fails with "no pull requests found")? Compare the branch against the repo's default branch on GitHub instead — `<branch>` is the branch the user named, else `git branch --show-current`. This needs no local remote and no fetch: `base=$(gh repo view --json defaultBranchRef -q .defaultBranchRef.name) && enc() { printf '%s' "$1" | sed 's/%/%25/g; s/#/%23/g'; } && gh api "repos/{owner}/{repo}/compare/$(enc "$base")...$(enc '<branch>')" --jq '{total_commits, commits:[.commits[].commit.message|split("\n")[0]], files:[.files[].filename]}'` — `enc` percent-encodes `%` and `#`, the only URL-significant characters Git allows in a ref. If `total_commits` exceeds the commits listed or exactly 300 files come back, GitHub capped the response: say the summary is based on a partial list. A 404 means `<branch>` isn't on GitHub yet: ask the user to push it, or summarize from a local `git log "$(gh repo view --json defaultBranchRef -q .defaultBranchRef.name)..<branch>" --oneline` and say the comparison is local. Open with "Here is what's on this branch:" and end with `Branch: <branch> (no PR yet)` instead of the PR line.
