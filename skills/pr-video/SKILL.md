...
- `gh` authenticated (`gh auth status`). **Playwright** is resolved from `--app-dir` — normally the target app's own copy, so nothing global. Two first-run snags, both with easy outs:
  - *Playwright's browser isn't downloaded* (the script says so and exits 3): run `npx playwright install chromium` in that package, **or** pass `--channel chrome` to record with the installed Google Chrome — no download.
